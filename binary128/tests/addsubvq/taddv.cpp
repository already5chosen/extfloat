...
void test(long long it0, long long it1, test_context* context)
{
  ...
  for (long long i = it0; i < it1; ++i) {
    ...
    uint64_t ra[RA_LEN*MAX_VLEN];
    for (int k = 0; k < RA_LEN*vlen; ++k)
      ra[k] = rndGen();

    __float128 x[MAX_VLEN], y[MAX_VLEN];
    for (int k = 0; k < vlen; ++k) {
      uint8_t  x_class = ra[RA_LEN*k+0] >> (8*0);
      uint8_t  y_class = ra[RA_LEN*k+0] >> (8*1);
      uint8_t  r_class = ra[RA_LEN*k+0] >> (8*2);

      int x_exp = make_float128(&x[k], &ra[RA_LEN*k+1], x_class, 0, 0x7FFE);
      int y_exp_min = 0;
      ...
      int y_exp = make_float128(&y[k], &ra[RA_LEN*k+3], y_class, y_exp_min, y_exp_max);
      ...
      if (!equal) {
        if (!mpfr_nan_p(ref) || !mpfr_nan_p(resx)) {
          ...
          if (context->nMism < 10) {
            ...
            quadmath_snprintf(xbuf, sizeof(xbuf), "%+-.28Qa", x[k]);
