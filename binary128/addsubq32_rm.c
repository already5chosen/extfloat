...
__float128 __addtf3(__float128 x, __float128 y)
{
      ...
      if (x1 < y1) {
        ...
        if (x0 == y0) {       // abs(x) == abs(y)
          if (xHiw != yHiw) { // x == -y => subtraction
            ...
            uint32_t resHi = 0;         // result of full cancellation => +zero
            if (rm == fast_FE_DOWNWARD) // except when rounding toward negative infinity
              resHi = BIT_31;           // -0
            return mk4_f128(resHi, 0, 0, 0);
          }
        } else if (x0 < y0) {
          swap = true;
        }
  ...
  if (delta_exp <= 14) {
    ...
    if (delta_exp > 114) {
      ...
      if (rm != fast_FE_TONEAREST) {
        if (rm == fast_FE_UPWARD) {
          ...
          // y = y < 0 ? -1 : 0
          y0 = (int32_t)y3 >> 31; // convert y3 to all '1' or all '0'. This code is not portable by C rules, but guaranteed to work in gcc
          y3 = y2 = y1 = y0;
        }
        add128(&xHiw,&x2,&x1,&x0, xHiw,x2,x1,x0, y3,y2,y1,y0);
      }
      return mk4_f128(xHiw, x2, x1, x0);
    }
    ...
    if (rm == fast_FE_TONEAREST) {
      ...
      if (resG != 0)
        res0 |= 1;
      incr128by32(&res3,&res2,&res1,&res0, BIT_15-1);
    }
