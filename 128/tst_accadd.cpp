...
static void make_random_pair(extfloat128_t::acc_t* dstA, extfloat128_t* dstB, std::mt19937_64& rndGen, std::uniform_int_distribution<uint64_t>& rndDistr);
...
static bool report_mismatch(extfloat128_t::acc_t A, extfloat128_t B, uint64_t n, int op);
int main(int argz, char** argv)
{
  if (argz < 2)
  {
    fprintf(stderr,
      "tst_accadd - validate implementation of acc_t:addsub.\n"
      "Usage:\n"
      "tst_accadd nIter\n"
      );
    return 1;
  ...
  boost_float384_t  refA, refB, refR, resR;
  uint64_t n = 1;
  while (nIter > 0) {
    extfloat128_t::acc_t A0;
    extfloat128_t        B;
    make_random_pair(&A0, &B, rndGen, rndDistr);

    convert_to_boost_bin_float(&refA, A0);
    convert_to_boost_bin_float(&refB, B);

    refR = refA + refB;
    extfloat128_t::acc_t A = A0;
    A += B;
    if (!isLegal(A)) {
  ...
  std::mt19937_64& rndGen, std::uniform_int_distribution<uint64_t>& rndDistr)
{
  ...
  for (int i = 0; i < bsl; ++i)
    dstB->m_significand[i] = rndFunc();
  uint64_t aux = rndFunc();

  const uint64_t BIT63 = (uint64_t(1) << 63);
  dstB->m_significand[1] |= BIT63;
  uint32_t bExp = uint32_t(aux);
  bExp = std::min(std::max(bExp, extfloat128_t::min_biased_exponent), extfloat128_t::max_biased_exponent);
  dstB->m_exponent = bExp;

  dstA->m_sign = (aux >> 32) & 1;
  dstB->m_sign = (aux >> 33) & 1;

  int deltaE = int((aux >> 34) & 0x7FF) - 0x400; // -1024 to 1023;
  int aNbits = ((aux >> 45) & 127) + dstA->NBITS_MIN+1;
  ...
  dstA->m_exponent = (dstA->exponent_bias - dstB->exponent_bias + bExp) + deltaE + 320 - aNbits;
