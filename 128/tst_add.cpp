...
static void speed_test(std::mt19937_64& rndGen, std::uniform_int_distribution<uint64_t>& rndDistr, int op);
...
static void print(const boost_quadfloat_t& a, const char* prefix = "") {
...
static void print(const boost_octafloat_t& a, const char* prefix = "") {
  printf("%s%d %11I64d", prefix, a.backend().sign(), a.backend().exponent());
  const size_t alen = a.backend().bits().size()*sizeof(a.backend().bits().limbs()[0]);
  if (alen == 32) {
    const uint64_t* bits = reinterpret_cast<const uint64_t*>(a.backend().bits().limbs());
...
static bool report_mismatch(extfloat128_t a, extfloat128_t b, uint64_t n, int op);
int main(int argz, char** argv)
{
  ...
  if (argz >= 10) {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      uint32_t e  = strtoul (argv[3+i*4], 0, 0);
  ...
  while (nIter > 0) {
    ...
    make_random_quadfloat(&res2, rndGen, rndDistr, false, &res1);
    ...
    if (!isEqual(res12, ref12)) {
      if (report_mismatch(res1, res2, n, '+'))
        return 1;
    }
    ...
    res12 = res2 - res1;
    if (!isEqual(res12, ref12)) {
      if (report_mismatch(res2, res1, n, '-'))
        return 1;
    }
...
static bool report_mismatch(extfloat128_t a, extfloat128_t b, uint64_t n, int op)
{
  boost_quadfloat_t ba, bb, abb;
  convert_to_boost_bin_float(&ba, a);
  convert_to_boost_bin_float(&bb, b);
  boost_quadfloat_t bab =  op=='-' ? ba - bb :  ba + bb;
  boost_octafloat_t oa(ba);
  boost_octafloat_t ob(bb);
  boost_octafloat_t oab = op=='-' ? oa - ob : oa + ob ;

  // first recheck, may be a bug was in boost
  if (bab == 0 && ba == 0 && bb == 0) { // patch
    if ((a.m_sign == b.m_sign && op=='-') || (a.m_sign != b.m_sign && op != '-')) {
      // difference of two zeros
      extfloat128_t ab0 = op=='-' ? a - b : a + b;
      if (ab0 == ab0.zero() && ab0.m_sign == 0)
        return false; // ignore mismatch in sign of difference of two zeros
    }
  }

  // yes the difference is real
  // set_div_dbg(1);
  extfloat128_t ab = op=='-' ? a - b : a + b;
  // set_div_dbg(0);
  convert_to_boost_bin_float(&abb, ab);
...
static void make_random_quadfloat(extfloat128_t* dst, std::mt19937_64& rndGen, std::uniform_int_distribution<uint64_t>& rndDistr, bool saneExponent, const extfloat128_t* pBuddy)
{
  ...
  if (saneExponent) {
    ...
    unsigned distE  = std::max((expMsw >> 7) & 15, 8u);
    unsigned distM  = 1u << distE;
    int dist = int(expLsw % (distM*2)) - distM;
    if      (dist > 130)  dist -= 128;
    else if (dist < -130) dist += 128;
    // generate "normal" exponent
    if (expSel == 0) {
      // zero
      msw  = lsw = 0;
      sign = 0;
      biased_exp = dst->zero_biased_exponent;
    } else {
      if ((expSel % 4) != 0 && pBuddy && isnormal(*pBuddy)) {
        // choose exponent close to buddy
        exp = pBuddy->_get_exponent() + dist;
      } else {
        // choose any exponent in "sane" range
        exp >>= 2;
      }
...
static void speed_test(std::mt19937_64& rndGen, std::uniform_int_distribution<uint64_t>& rndDistr, int op)
{
  ...
  //printf("sizes %I64u %I64u %I64u %I64u\n", sizeof(inpvec_a), sizeof(inpvec_b), sizeof(outvec_a), sizeof(outvec_b));
  uint64_t dummy[3] = {0};
  uint64_t dt_a[NITER];
  uint64_t dt_b[NITER];
  for (int it = 0; it < NITER; ++it) {
    make_random_quadfloat(&inpvec_a[0], std::ref(rndGen), rndDistr, true);
    for (int i = 1; i < VECLEN+1; ++i)
      make_random_quadfloat(&inpvec_a[i], std::ref(rndGen), rndDistr, true, &inpvec_a[i-1]);
    uint64_t t0 = __rdtsc();
    if (op == '-') {
  ...
  printf("%c my %I64u. boost %I64u\n", op, dt_a[NITER/2]/VECLEN, dt_b[NITER/2]/VECLEN);
