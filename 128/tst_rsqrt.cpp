...
int main(int argz, char** argv)
{
  ...
  {
    fprintf(stderr,
      "tst_rsqrt - validate implementation of rsqrt().\n"
      "Usage:\n"
      "tst_rsqrt nIter\n"
      );
    return 1;
  ...
  boost_decafloat_t one(1);
  boost_decafloat_t ref;
  ...
  if (argz >= 6) {
    ...
    m1 |= uint64_t(1) << 63;
    make_quadfloat(&res, s & 1, res.exponent_bias, m1, m0);
    switch (argv[3][0]) {
      case 'n':
      case 'i':
        res.m_exponent = res.inf_nan_biased_exponent;
        break;
      case 'z':
        res.m_exponent = res.zero_biased_exponent;
        break;
      default:
        res._set_exponent(e);
        break;
    ...
    // set_div_dbg(1);
    extfloat128_t rres = res.rsqrt();
    // set_div_dbg(0);

    convert_to_boost_bin_float(&ref, res);
    boost_decafloat_t rrefx = one/sqrt(boost_decafloat_t(ref));
    boost_quadfloat_t rref  = boost_quadfloat_t(rrefx);
    if (!isEqual(rres, rref)) {
      if (report_mismatch(res, 0))
        return 1;
    }
    print(boost_quadfloat_t(ref));
    print(rref);
    print(rres);
    print(rrefx);
    if (s > 1)
  ...
  while (nIter > 0) {
    ...
    boost_quadfloat_t rref = boost_quadfloat_t(one/sqrt(ref));
    extfloat128_t     rres = res.rsqrt();
    if (!isEqual(rres, rref)) {
...
static bool report_mismatch(extfloat128_t a, uint64_t n)
{
  ...
  extfloat128_t       ra = a.rsqrt();
  boost_quadfloat_t rb = 1/sqrt(b);
  boost_decafloat_t o(b);
  boost_decafloat_t ro = 1/sqrt(o);
...
static void speed_test(std::mt19937_64& rndGen, std::uniform_int_distribution<uint64_t>& rndDistr)
{
  ...
  for (int it = 0; it < NITER; ++it) {
    ...
    t0 = __rdtsc();
    for (int i = 0; i < VECLEN; ++i)
      outvec_b[i] = one/sqrt(inpvec_b[i]);
    t1 = __rdtsc();
    dt_b[it] = t1 - t0;
    for (int i = 0; i < VECLEN; ++i) {
