...
int main(int argz, char** argv)
{
  ...
  {
    fprintf(stderr,
      "tst_asin - validate implementation of asinPi().\n"
      "Usage:\n"
      "tst_asin nIter [u | r | b]\n"
      "where\n"
      " u - test with arguments uniformly distributed in range (-1..1)\n"
      " r - test with results uniformly distributed in range (-1/2..1/2)\n"
      " b - test boost implementation with results uniformly distributed in range (-1/2..1/2)\n"
      );
    return 1;
  }
  ...
  const double errThr = testBoost ? 64: 15./2;
  ...
  for (int64_t n = 0;n < nIter; ++n) {
    ...
    if (!testBoost) {
      ...
      boost_quadfloat_t yRes_q = asin(boost_quadfloat_t(x_o))/qPi;
    ...
    if (absx <= 1) {
      extStatx_i = std::min(int(trunc(absx*EXTENDED_STAT_N).convert_to_double()), EXTENDED_STAT_N);
      iter_vTab[0][extStatx_i] += 1;
    }
    if (isfinite(yRef)) {
      extStaty_i = std::min(int((trunc(abs(yRef)*(EXTENDED_STAT_N*2))).convert_to_double()), EXTENDED_STAT_N);
...
static bool report_mismatch(extfloat128_t a, int64_t n)
{
  ...
  extfloat128_t     ra = asinPi(a);
  boost_quadfloat_t rb = asin(b)/boost::math::constants::pi<boost_quadfloat_t>();
  boost_octafloat_t ro = asin(o)/boost::math::constants::pi<boost_octafloat_t>();
  ...
  int  distrib)
{
  ...
  if (distrib == 'u') {
    ...
    static const double SCALE48 = 5.5805895968138108216e-015;           // pi/2/2^48
    static const double SCALE63 = 5.5805895968138108216e-015/(1 << 15); // pi/2/2^63
    extfloat128_t dst;
    if (exw > (1u << 20))
      dst = sin(int64_t(exw >> 16)*SCALE48); // argument of sin uniformly distributed in range [0..pi/2)
    else
